...
def sender_program(fieldLength=6, frameLostProb=-1):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((HOST, PORT))
    address = "C:\\Users\\NM\\Documents\\Python\\SRA\\text.txt"
    window = Window(fieldLength)
    frameManager = FrameManager(client_socket, address, window, frameLostProb)
...
class Frame:
    def __init__(self, sequenceNumber, data=None):
    ...
    def run(self):
        ...
        while self.packetCount < len(self.frames):
            ...
            if len(self.window.transmittedFrames.keys()) < self.window.maxWindowSize:
                print(f"Client is sending packet {self.packetCount % 2 ** self.window.dataSize} \n")
                with LOCK:
                ...
                while self.frameManager.nextFrame2Send != seqNum:
                    with LOCK:
                        ...
                        self.frameManager.packetCount -= (self.frameManager.packetCount % (
                                    2 ** self.window.dataSize) - seqNum) if self.frameManager.packetCount % (
                                    2 ** self.window.dataSize) > seqNum else (
                                    seqNum + 2 ** self.window.dataSize - self.frameManager.packetCount % (
...
if __name__ == '__main__':
    print(f"[STARTING] Sender is starting to send ...")
    sender_program()
